...
def file_prompt():
    ...
    while not inp:
        try:
            inp = input(
                "Step 1: Please drag and drop plate file (CSV file only). For this step only, type 'quit' to exit program: ")
...
def main():
    ...
    while count < num_quads:
        try:
            ...
            half_log_prompt = input(
                "Step 5 for Quadrant " + str(count + 1) + ": Are you using log (input y) or half-log (input n): ")
            ...
            print(
                "Warning: discrepancy between the minimum concentration and rows of controls. Please input information again for this quadrant.\n")
